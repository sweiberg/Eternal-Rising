...
#define ER_SERVER_VERSION "1.0.0.0"

// UDP port for the ER server to listen on
#define ER_SERVER_PORT 27015

// UDP port for the master server updater to listen on
#define ER_MASTER_SERVER_UPDATER_PORT 27016
...
struct ClientERUpdateData_t
{
	void SetPlayerName( const TCHAR *pchName ) { FCString::Strncpy( m_rgchPlayerName, pchName, UE_ARRAY_COUNT( m_rgchPlayerName ) ); }
	const TCHAR *GetPlayerName() { return m_rgchPlayerName; }
